...
def send_salebot_callback(client_id, direct_url):
    """
    Отправка callback в Salebot с ссылкой на микс.
    Теперь используется правильные поля:
    client_id и callback_message
    """
    try:
        payload = {
            "client_id": client_id,           # идентификатор клиента Salebot
            "callback_message": direct_url    # ссылка на микс
        }
        resp = requests.post(
